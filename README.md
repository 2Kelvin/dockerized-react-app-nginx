...
In the first stage `react_build`, that's where all the heavy building of the app is happening, which ends with a production build folder. In stage 2 `web_server`, I copied the build folder from **react_build** into this stage to be served by the `Nginx` container.
